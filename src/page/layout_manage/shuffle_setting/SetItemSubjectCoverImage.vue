<!--内容管理-栏目管理-设置混排模块内的专题第二步--选择专题图片组件-->
<template>
    <div>
        ...
        <ul class="cover-list">
            <li v-for="(item,index) in specPosterImages" :key="index">
                ...
                <el-radio v-model="subjectImageUri" :label="item.uri" @change="setCoverImage">{{item.name}}
                </el-radio>
            </li>
        </ul>
        <div class="add-box">
...
<script>
    ...
    export default {
        name: 'SetItemSubjectCoverImage',
        components: {
            UploadImage,
            PreviewMultipleImages
        },
        props: ['subject', 'imageSpec', 'originState'],
        data() {
            return {
                size: [],
                subjectImageUri: '',
        ...
        computed: {
            specPosterImages() {
                return this.subject.posterImageList.filter(image => parseInt(image.width) === this.imageSpec.width && parseInt(image.height) === this.imageSpec.height);
            }
        },
        mounted() {
            this.init();
        ...
        methods: {
            init() {
                ...
                if (this.originState && this.originState.coverImage) {
                    this.subjectImageUri = this.originState.coverImage.uri;
                }
            },
            initImageSize() {
            ...
            setCoverImage() {
                ...
                this.specPosterImages.map(image => {
                    if (image.uri === this.subjectImageUri) {
            ...
            addPosterImage(newPosterImage) {
                ...
                let imageList = this.subject.posterImageList.slice();
                ...
                this.$service.updateSubjectBasicInfo({
                    ...
                    if (response && response.code === 0) {
                        this.subject.posterImageList.push(newPosterImage.posterImage);
                        // 设置新添加的图片为选中状态
                        this.subjectImageUri = newPosterImage.posterImage.uri;
